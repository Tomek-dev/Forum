...
public class Start {
    ...
    public Start(UserDao userDao, PasswordEncoder passwordEncoder, TopicDao topicDao) {
        ...
        Topic[] testTopics = new Topic[10];

        testTopics[0] = new Topic();
        testTopics[0].setUser(user[0]);
        user[0].getTopics().add(testTopics[0]);
        testTopics[0].setType(Type.JAVA);
        testTopics[0].setCreatedAt(new Date(date-60000));
        testTopics[0].setTitle("How to get streaming data from dolphindb with java api？");
        testTopics[0].setDescription("I am making a streaming data testing in a dolphindb cluster , there is a streaming table in a data node, i subscribed the streaming data with java api but no data received. i use ThreadedClient to subscribe streamTable and no exceptions, could anyone give a hand?");

        testTopics[1] = new Topic();
        testTopics[1].setUser(user[3]);
        user[3].getTopics().add(testTopics[1]);
        testTopics[1].setType(Type.JAVA);
        testTopics[1].setCreatedAt(new Date(date-540000));
        testTopics[1].setTitle("Data not saved in db when the time @PostPersist got called");
        testTopics[1].setDescription("i need to send a request to other microservice once the object got created in the database. i only send the object id so other microservice needs to call the db again for the info with bunch of other stuff.\n" +
                "\n" +
        ...
        testTopics[2] = new Topic();
        testTopics[2].setUser(user[6]);
        user[6].getTopics().add(testTopics[2]);
        testTopics[2].setType(Type.JAVA);
        testTopics[2].setCreatedAt(new Date(date-340000));
        testTopics[2].setTitle("Add Lombok to a Gradle project in Intellij");
        testTopics[2].setDescription("New to Gradle. I Apologize if this is a bad question.\n" +
                "\n" +
        ...
        testTopics[3] = new Topic();
        testTopics[3].setUser(user[9]);
        user[9].getTopics().add(testTopics[3]);
        testTopics[3].setType(Type.JAVA);
        testTopics[3].setCreatedAt(new Date(date-14000));
        testTopics[3].setTitle("Spring @valid annotation cannot be resolved");
        testTopics[3].setDescription("I am trying to create a simple JPA framework that performs simple CRUD operations like save employees, delete employees or get a simple employee.");

        testTopics[4] = new Topic();
        testTopics[4].setUser(user[4]);
        user[4].getTopics().add(testTopics[4]);
        testTopics[4].setType(Type.JAVA);
        testTopics[4].setCreatedAt(new Date(date-170000));
        testTopics[4].setTitle("Applying foreground color / highlight to specific rows of the excel using Apache POI");
        testTopics[4].setDescription("Am copying data/ sheet from one excel workbook to another workbook using copyRow. I want to apply foreground color to specific rows in my target excel. Below is the code i have now. Please help how should i do it. Found the link for Cell Style from POI's quick guide. But please help to use it to apply it to specific rows in the target excel.");

        testTopics[5] = new Topic();
        testTopics[5].setUser(user[2]);
        user[2].getTopics().add(testTopics[5]);
        testTopics[5].setType(Type.JAVA);
        testTopics[5].setCreatedAt(new Date(date-78000));
        testTopics[5].setTitle("Elasticsearch composite group by queries across the documents");
        testTopics[5].setDescription("We have an elastic search document which has a dimension called city. Each document will have only one value for city field. I have a scenario where I need to query the person based on the city or cities.");

        testTopics[6] = new Topic();
        testTopics[6].setUser(user[7]);
        user[7].getTopics().add(testTopics[6]);
        testTopics[6].setType(Type.JAVA);
        testTopics[6].setCreatedAt(new Date(date-600000));
        testTopics[6].setTitle("How to use internationalization in struts2 for dropdownlist");
        testTopics[6].setDescription("By using struts2 localization how to change dropdowmlist value english to hindi");

        testTopics[7] = new Topic();
        testTopics[7].setUser(user[1]);
        user[1].getTopics().add(testTopics[7]);
        testTopics[7].setType(Type.JAVA);
        testTopics[7].setCreatedAt(new Date(date-650000));
        testTopics[7].setTitle("Setting objects as partners within the same class in java");
        testTopics[7].setDescription("As a task in my beginners course in object oriented programming, I must set two obects in my Partner class as \"married\". This is my attempt at beginning: public class Partner {");

        testTopics[8] = new Topic();
        testTopics[8].setUser(user[8]);
        user[8].getTopics().add(testTopics[8]);
        testTopics[8].setType(Type.JAVA);
        testTopics[8].setCreatedAt(new Date(date-120000));
        testTopics[8].setTitle("Logging Request and Response BODY of Spring WebClient");
        testTopics[8].setDescription("I would like to log all request and response text payload of WebClient used in a Spring WebFlux project and have gone through a few solutions.\n" +
                "\n" +
        ...
        testTopics[9] = new Topic();
        testTopics[9].setUser(user[0]);
        user[0].getTopics().add(testTopics[9]);
        testTopics[9].setType(Type.JAVA);
        testTopics[9].setCreatedAt(new Date(date-40000));
        testTopics[9].setTitle("The module has not been deployed.- RESTFUL WEB SERVICE - GLASSFISH SERVER 4.0");
        testTopics[9].setDescription("I am trying to create simple restful webservice in netbeans. Everything is going well until I connect my webservice to JDBC and is giving me some runtime exception. I've been using Netbeans 8.0.2 with glassfish 4.0. does anyone encounter the same problem?\n" +
                "\n" +
                "I have tried task manager-> proccess->end process Java SE but no to avail.");

        topicDao.saveAll(Arrays.asList(testTopics));
